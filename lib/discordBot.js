...
// In-memory set tracking thank-you interactions that have already been sent.
// Entries are in the format `${messageId}:${userId}` to prevent duplicate thanks.
const sentThanks = new Set();
const SENT_THANKS_MAX_SIZE = 10_000;

/**
 * Add a key to sentThanks. When the set reaches SENT_THANKS_MAX_SIZE,
 * the oldest half of entries are pruned to keep memory bounded.
 * @param {string} key
 */
function recordThanks(key) {
  if (sentThanks.size >= SENT_THANKS_MAX_SIZE) {
    const pruneCount = Math.floor(SENT_THANKS_MAX_SIZE / 2);
    let deletedCount = 0;
    for (const entry of sentThanks) {
      if (deletedCount++ >= pruneCount) break;
      sentThanks.delete(entry);
    }
  }
  sentThanks.add(key);
}
...
async function initBot() {
  ...
  botClient.on('interactionCreate', async (interaction) => {
    if (interaction.isButton() && interaction.customId.startsWith('thank_sender:')) {
      ...
      if (senderUserId) {
        const thanksKey = `${interaction.message.id}:${interaction.user.id}`;
        if (sentThanks.has(thanksKey)) {
          await interaction.reply({
            content: '⚠️ You have already sent your thanks for this PictoCard!',
        ...
        try {
          ...
          await interaction.reply({
            content: 'Could not send your thanks. Please try again.',
            flags: MessageFlags.Ephemeral
          });
        }
      }
    ...
    if (interaction.commandName === 'optout') {
      ...
      await interaction.reply({
        content:
          '✅ You have opted out of receiving PictoCards via Discord. Use `/optin` to reverse this.',
        flags: MessageFlags.Ephemeral
      });
  ...
  await new Promise((resolve, reject) => {
    const timeout = setTimeout(
      () => reject(new Error('Discord bot ready timeout after 30s')),
 ...
 */
async function sendCardToDiscordUser(
  username,
  senderDisplayName,
  message,
  cardImageBuffer,
  ...
  if (isUserOptedOut(username)) {
    return {
      success: false,
      error: 'That user has opted out of receiving cards via Discord.'
    };
  }
  ...
  try {
    ...
    if (!targetUser) {
      return {
        success: false,
        error: `Could not find Discord user "${username}" in any shared server.`
      };
    }

    const dmChannel = await targetUser.createDM();
    const attachment = new AttachmentBuilder(cardImageBuffer, {
      name: 'pictocard.png'
    });
...
async function sendVerificationCodeViaDM(username, code, userId = null) {
  if (!botClient || !botReady) {
    return {
      success: false,
      error: 'Discord bot is not connected. Make sure you have invited the bot to a shared server.'
    };
  }
  ...
  try {
    ...
    if (userId) {
      ...
      try {
        ...
        console.warn(
          '[Discord] Failed to fetch user by ID, falling back to guild search:',
          err.message
        );
      }
    }
    ...
    if (!targetUser) {
      return {
        success: false,
        error: `Could not find Discord user "${username}" in any shared server. Make sure the bot is in a server with you.`
      };
    }
 ...
 */
async function sendConfirmationViaDM(
  senderUsername,
  recipientUsername,
  cardImageBuffer,
  senderUserId = null
  ...
  try {
    ...
    if (senderUserId) {
      try {
        ...
        console.warn(
          '[Discord] Failed to fetch sender by ID, falling back to guild search:',
          err.message
        );
      }
    }
