...
export class MiiToStudio {
  ...
  getImageUrl(width, allBody) {
    switch (width) {
      ...
      default:
        if (
          (width != 128 || allBody) &&
          'https://mii-unsecure.ariankordi.net/miis/image.png?data='.startsWith(
            'https://studio.mii.nintendo.com/'
          )
        ) {
    ...
    for (let i = 0; i < 47; i++) dataHex += `${urlData[i].toString(16).padStart(2, '0')}`;
  ...
  static _convertFieldsVer3ToNx(buf) {
    if (buf[27] == 0) buf[27] = 8;
    if (buf[0] == 0) buf[0] = 8;
    if (buf[11] == 0) buf[11] = 8;
    buf[36] += 19;
    buf[4] += 8;
    if (buf[23] == 0) buf[23] = 8;
    else if (buf[23] < 6) buf[23] += 13;
    if (127 < buf[2]) buf[2] = 127;
    if (127 < buf[30]) buf[30] = 127;
  }
