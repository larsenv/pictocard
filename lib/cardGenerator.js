...
// Text section is 203px wide when the image section is 313px wide (516px total card)
// i.e. textWidth / (imgWidth + separator) = 203/313
const TEXT_SECTION_RATIO = 203 / 313;
...
async function generateCard({ imageBuffer, text, font, textColor, senderName, miiData, includeDate }) {
  ...
  const SEPARATOR_W = 3;
  const TEXT_SECTION_WIDTH = Math.round((imgSectionWidth + SEPARATOR_W) * TEXT_SECTION_RATIO);
  ...
  if (miiData) {
    ...
    if (result) {
        const miiUrl = converter.getImageUrl(170, false);
        ...
        if (miiBuf) {
          try {
            ...
            ctx.drawImage(miiImg, cardWidth - miiW + 25, cardHeight - miiH - 7, miiW, miiH);
          } catch {
