...
const TWEMOJI_BASE = (
  config.twemojiCdnBase || 'https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/72x72'
).replace(/\/$/, '');
...
// node-canvas's loadImage() cannot decode WebP. Detect the RIFF/WEBP signature
// and transcode to PNG via sharp first so uploads in that format still work.
...
async function ensureCanvasCompatible(buffer) {
  if (isWebp(buffer)) {
...
async function generateCard({
  ...
  const uploadedImage = await loadImage(await ensureCanvasCompatible(imageBuffer));
  ...
  const SEPARATOR_W = 3;
  const TEXT_SECTION_WIDTH = Math.round(
    (TEXT_SECTION_REF_WIDTH * cardHeight) / TEXT_SECTION_REF_HEIGHT
  );
  ...
  const resolvedTextColor =
    textColor && /^#[0-9a-fA-F]{3,8}$/.test(textColor) ? textColor : '#111111';
    ...
    if (result && converter.isValid()) {
      ...
      if (miiBuf) {
        try {
          ...
          ctx.drawImage(
            miiImg,
            cardWidth - miiW + miiOffsetX,
            cardHeight - miiH - miiOffsetY,
            miiW,
            miiH
          );
        } catch {
