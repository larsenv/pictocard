...
async function generateCard({ imageBuffer, text, font, textColor, senderName, miiData, includeDate }) {
  ...
  // Image section = top 60% of the 1920px tall card.
  ...
  const textSectionTop = imageSectionHeight + 3; // just below separator
  const textSectionHeight = CARD_HEIGHT - textSectionTop;

  // ---- Create canvas ----
  const canvas = createCanvas(cardWidth, CARD_HEIGHT);
  const ctx = canvas.getContext('2d');

  // Dark background fills the whole card; text section will be overdrawn in white.
  ctx.fillStyle = BG_COLOR;
  ctx.fillRect(0, 0, cardWidth, CARD_HEIGHT);

  // ---- Draw uploaded image (top section, centred) ----
  ...
  ctx.fillRect(0, textSectionTop, cardWidth, textSectionHeight);

  // ---- Date stamp (top-right of image area) ----
  ...
  // Bigger font; text color defaults to near-black on the white background
  const fontSize = 56;
  const lineHeight = fontSize * 1.45;
  const resolvedTextColor = textColor && /^#[0-9a-fA-F]{3,8}$/.test(textColor) ? textColor : '#111111';

  // Text protrudes to the right: anchor to right edge with a margin.
  const rightMargin = 48;
  const textRightEdge = cardWidth - rightMargin;  // text is right-aligned up to here
  const textLeftEdge = textRightEdge - TEXT_SECTION_WIDTH;
  ...
  // Render lines right-aligned: measure each line's total width and start from right.
  let cursorY = textSectionTop + 32;
  for (const line of limitedLines) {
    // Compute total line width for right-alignment starting x
    let lineWidth = 0;
    for (const token of line) {
    ...
    for (const token of line) {
      ...
      if (token.type === 'emoji') {
        ...
        if (img) {
          ...
          // Fallback: draw emoji character with left-baseline
          ctx.save();
          ctx.textAlign = 'left';
          ctx.fillStyle = resolvedTextColor;
  ...
  // ---- Sender name (bottom-right of text section) ----
  if (senderName) {
    ctx.save();
    ctx.font = `italic ${Math.round(fontSize * 0.7)}px "${chosenFont}", Whitney, Helvetica, sans-serif`;
    ctx.fillStyle = '#666666';
    ctx.textAlign = 'right';
    ctx.textBaseline = 'bottom';
    ctx.fillText(`- ${senderName}`, textRightEdge, CARD_HEIGHT - 32);
    ctx.restore();
  }

  // ---- Mii (bottom-right corner, overlaid on text section) ----
  if (miiData) {
