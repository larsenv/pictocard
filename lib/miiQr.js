...
function decryptMiiQrPayload(rawQrBytes) {
  ...
  try {
    ...
    const ciphertext = rawQrBytes.slice(8, 96); // 88 bytes

    // Build the CTR IV for CCM message encryption (counter block #1).
    // flags byte = L − 1 = 2  (L = 3 because nonce is 12 bytes)
    // IV layout (16 bytes):
    //   byte  0   : 0x02 (flags)
    //   bytes 1–8 : 8-byte nonce
    //   bytes 9–12: zero padding (extends nonce to 12 bytes)
    //   bytes 13–14: zero (high bytes of 3-byte big-endian counter = 1)
    //   byte 15   : 0x01 (low byte of counter)
    ...
    // Reconstruct the FFLStoreData layout:
    //   plaintext[0:12] + nonce(8) + plaintext[12:]  →  96-byte Mii binary
    ...
    console.log(`[miiQr] AES-CCM decrypt error: ${String(err.message || err).replace(/[\r\n]/g, ' ').slice(0, 200)}`);
    return null;
  }
}
...
async function decodeQrWithJsQr(imageBuffer) {
  try {
    ...
    console.log(`[miiQr] jsqr: QR code found, payload length=${code.data.length}`);
    ...
    console.log(`[miiQr] jsqr: decoded ${bytes.length} bytes from QR payload`);
    ...
    if (bytes.length === 96) {
      console.log('[miiQr] 96-byte payload detected — attempting AES-CCM decrypt (3DS/Wii U QR format)');
